...
function updateStaticAppVersion(){
  const target='PRO 4.9.5 · 29.08.2026';
...
function render(){try{app.innerHTML=(views[state.view]||views.home)();bind();setupScrollUi();updateStaticAppVersion();updateAuthUI().catch(console.error);if(state.view==='study'&&state.mode==='speaking')setTimeout(startSpeaking,250)}catch(e){console.error(e);app.innerHTML=`<section class="panel"><h2>Сталася помилка</h2><p>${esc(e.message)}</p><button class="btn primary" data-act="home">На головну</button></section>`;bind()}}
const views={
  home:()=>`
  <section class="panel">
    <div class="home">
      <div class="version-user-row" style="display:flex;align-items:center;justify-content:flex-end;gap:10px;position:relative;margin-bottom:8px">
<button
          id="userAvatarButton"
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=4.9.5',{updateViaCache:'none'}).catch(console.error))}
