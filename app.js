...
function updateStaticAppVersion(){
  const target='PRO 5.0.9 · 23.09.2026';
...
function shell(title,body,back){const right=state.view==='detail'?`<button class="home-mini folder-share-top" data-act="share-folder" type="button" aria-label="Поділитися папкою" title="Поділитися папкою">↔</button>`:'<span></span>';return `<section class="panel"><div class="top">${back?`<button class="back" data-act="${back}">← Назад</button>`:'<span></span>'}<h2>${esc(title)}</h2>${right}</div>${body}</section>`}
...
const views={
  ...
  <section class="panel">
    <div class="home">
      ...
      <div class="app-footer-version">PRO 5.0.9 · 23.09.2026</div>
    </div>
  </section>
`,
    ...
    extras.push(`<div class="expanded-notes-editor" style="margin-top:10px">
      <div class="notes-title-row">
        ...
        <div class="notes-actions">
          <button class="smallbtn" data-act="speak-note" data-id="${esc(w.id)}" type="button" title="Прослухати нотатку">🔊</button>
          <button class="smallbtn" data-act="edit-note-inline" data-id="${esc(w.id)}" type="button" title="Редагувати нотатку">✏️</button>
        </div>
      </div>
      <div class="expanded-note-text" style="white-space:pre-wrap">${w.notes?esc(w.notes):'<span class="muted">Нотатки немає.</span>'}</div>
    </div>`);
...
function bind(){app.onclick=handleClick; const headerHome=document.getElementById('headerHomeButton');if(headerHome&&!headerHome.dataset.bound){headerHome.dataset.bound='1';headerHome.addEventListener('click',()=>{state.bulkMove=false;state.selectedWordIds=[];state.expandedWordId=null;setView('home')})} const loginForm = document.getElementById('loginForm');
...
async function refreshEnglishTranslation(){const ko=$('#ko')?.value.trim(),en=$('#en'),status=$('#autoFillStatus');if(!ko||!en){if(status)status.textContent='Спочатку введи корейське слово.';return}if(status)status.textContent='🔄 Оновлюю англійський переклад…';const translated=await translateToEnglish(ko);if(translated){en.value=translated;if(status)status.textContent='Готово ✓'}else if(status)status.textContent='Переклад не знайдено.'}
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=5.0.9',{updateViaCache:'none'}).catch(console.error))}
