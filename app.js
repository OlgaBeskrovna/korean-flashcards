...
document.title='Korean Flashcards';
...
const APP_ASSET_BASE='https://cqikjyakfnkrxlrfvxgp.supabase.co/storage/v1/object/public/app-assets';
const DEFAULT_AVATARS=['avatar-1.png','avatar-2.png','avatar-3.png','avatar-4.png','avatar-5.png','avatar-6.png'];
function appAsset(name){return `${APP_ASSET_BASE}/${encodeURIComponent(name)}?v=5.0.9`}
...
function updatePwaIcon(theme=currentTheme()){
  const icon=appAsset(theme==='pink'?'icon-pink.png':'icon-purple.png');
...
function selectedAvatar(){
  ...
  return selected==='custom'&&custom?custom:appAsset(DEFAULT_AVATARS.includes(selected)?selected:'avatar-1.png');
}
...
async function setCloudThemeValue(theme){
  const clean=theme==='pink'?'pink':'purple';
  const {data,error}=await supabaseClient.rpc('set_my_theme_v5',{p_theme:clean});
  if(error)throw error;
  const row=rpcProfileRow(data);
  if(row)socialProfile={...(socialProfile||{}),...row};
...
function hydrateAvatarFromCloud(){
  const value=String(socialProfile?.avatar_value||'').trim();
  if(/^https?:\/\//i.test(value)){
    localStorage.setItem(customAvatarStorageKey(),value);
    localStorage.setItem(avatarStorageKey(),'custom');
  }else if(DEFAULT_AVATARS.includes(value)){
    localStorage.setItem(avatarStorageKey(),value);
    localStorage.removeItem(customAvatarStorageKey());
  }
  refreshAvatarButton();
}
async function uploadCustomAvatarBlob(blob){
  ...
  // A new object uses the INSERT policy only. The first path segment must be auth.uid().
...
async function loadSocialData(){
  ...
  try{
    const {data:profileData,error:profileError}=await supabaseClient.rpc('ensure_my_profile_v5');
    if(profileError)throw profileError;
    const profile=rpcProfileRow(profileData);
    if(!profile?.nickname)throw new Error('Nickname missing from profile RPC');
    socialProfile=profile;

    // Cloud profile is the source of truth after reinstall/login.
    // For users upgrading from older releases, migrate the local theme once if the cloud field is still empty.
    if(profile.theme==='pink'||profile.theme==='purple'){
      localStorage.setItem(APP_THEME_KEY,profile.theme);
      applyAppTheme();
    }else{
      await setCloudThemeValue(currentTheme());
    }
    ...
    // One-time migration for a custom avatar that existed only on this device in older releases.
    const selected=localStorage.getItem(avatarStorageKey())||'avatar-1.png';
    const custom=localStorage.getItem(customAvatarStorageKey());
    const cloudAvatar=String(socialProfile.avatar_value||'');
    if(selected==='custom'&&custom?.startsWith('data:image/')&&!/^https?:\/\//i.test(cloudAvatar)){
...
async function randomizeNickname(){
  const {data,error}=await supabaseClient.rpc('randomize_my_nickname_v5');
  if(error){alert('Не вдалося підібрати нікнейм: '+error.message);return}
  const row=rpcProfileRow(data);
  if(row)socialProfile={...(socialProfile||{}),...row};
  openProfileModal();
}
async function copyUserId(){
  ...
  try{
    ...
    const status=document.querySelector('.copy-user-id-status');
    if(status){status.textContent=' Скопійовано';setTimeout(()=>{if(status)status.textContent=''},1400)}
  }catch(error){console.warn(error);alert('Не вдалося скопіювати User ID.')}
}
...
async function openProfileModal(){
  ...
  modal.innerHTML=`<div class="profile-modal" role="dialog" aria-modal="true" aria-label="Профіль">
    ...
    <div class="profile-identity">
      ...
      <div class="profile-nickname">@${esc(socialProfile?.nickname||'...')} <button class="nickname-edit" type="button" title="Змінити нікнейм">✏️</button> <button class="nickname-random" type="button" title="Випадковий нікнейм" aria-label="Випадковий нікнейм">🎲</button></div>
      <div class="profile-user-id">User ID: <code>${esc(currentUserId||'')}</code> <button class="copy-user-id" type="button" title="Копіювати User ID">📋</button><span class="copy-user-id-status" aria-live="polite"></span></div>
    </div>
    ...
    <div class="avatar-grid">
      ${DEFAULT_AVATARS.map((name,i)=>`<button class="avatar-choice ${selected===name?'selected':''}" data-avatar="${name}" type="button" aria-label="Аватар ${i+1}">
        <img src="${appAsset(name)}" alt="Аватар ${i+1}">
  ...
  modal.querySelector('.nickname-random')?.addEventListener('click',randomizeNickname);
  modal.querySelector('.copy-user-id')?.addEventListener('click',copyUserId);
  ...
  modal.querySelectorAll('.avatar-choice').forEach(btn=>{
    btn.addEventListener('click',async()=>{
      ...
      try{
        await setCloudAvatarValue(name);
        localStorage.setItem(avatarStorageKey(),name);
        localStorage.removeItem(customAvatarStorageKey());
        refreshAvatarButton();
        const img=modal.querySelector('.profile-current-avatar img');if(img)img.src=getAvatarSrc();
        modal.querySelectorAll('.avatar-choice').forEach(x=>x.classList.toggle('selected',x.dataset.avatar===name));
      }catch(error){
        console.error('Avatar cloud sync failed',error);
        alert('Не вдалося зберегти аватарку в профілі. Спробуйте ще раз.');
      }
    });
  });

  modal.querySelectorAll('[data-profile-theme]').forEach(btn=>btn.addEventListener('click',async()=>{
    const theme=btn.dataset.profileTheme==='pink'?'pink':'purple';
    try{
      await setCloudThemeValue(theme);
      refreshAvatarButton(); close(); render(); openProfileModal();
    }catch(error){
      console.error('Theme cloud sync failed',error);
      alert('Не вдалося зберегти тему в профілі. Спробуйте ще раз.');
    }
  }));
  ...
  input?.addEventListener('change',e=>{
    ...
    reader.onload=async()=>{
      try{
        const publicUrl=await uploadCustomAvatarBlob(file);
        localStorage.setItem(customAvatarStorageKey(),publicUrl);
        localStorage.setItem(avatarStorageKey(),'custom');
        refreshAvatarButton();
        modal.querySelectorAll('.avatar-choice').forEach(x=>x.classList.remove('selected'));
        let preview=modal.querySelector('.custom-avatar-preview');if(!preview){preview=document.createElement('div');preview.className='custom-avatar-preview selected-custom';modal.querySelector('.avatar-upload')?.insertAdjacentElement('afterend',preview)}preview.innerHTML=`<img src="${publicUrl}" alt="Моя аватарка"><span class="avatar-check">✓</span>`;
      }catch(error){
        console.error('Avatar upload failed',error);
        alert('Не вдалося завантажити аватарку в хмару: '+(error?.message||'невідома помилка'));
      }
    };
    ...
    <div class="theme-picker">
      ...
      <div class="theme-options">
        <button class="theme-card ${theme==='purple'?'selected':''}" data-act="set-theme" data-theme="purple" type="button">
          <img src="${appAsset('icon-purple.png')}" alt="">
        ...
        <button class="theme-card pink ${theme==='pink'?'selected':''}" data-act="set-theme" data-theme="pink" type="button">
          <img src="${appAsset('icon-pink.png')}" alt="">
...
function ensureThemeStyles(){
  ...
  style.textContent=`
    ...
    .match-help{text-align:center;color:#7c748e;margin:4px 0 18px}.match-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:12px 14px;align-items:stretch}.match-column{display:grid;gap:12px;align-content:start}.match-card{width:100%;min-height:92px;border:2px solid #e7e1f2;border-radius:20px;background:#fff;color:#29233c;padding:12px 8px;font-size:clamp(16px,4.3vw,22px);font-weight:750;line-height:1.25;transition:.15s;overflow-wrap:anywhere}.match-card.selected{background:#eee8ff;border-color:#704be4;color:#5e3ed0}.theme-pink .match-card.selected{background:#fff0f5;border-color:#df6f9b;color:#c94f7e}.match-card.matched{background:#e6f8ef!important;border-color:#27b878!important;color:#148457!important}.match-card.wrong{background:#ffe9ea!important;border-color:#e6535c!important;color:#bd3139!important}.match-card:disabled{opacity:1}.more-menu{display:grid;gap:14px}.duplicate-badge{font-size:.72rem;font-weight:800;color:#d93645;margin-top:7px}.notes-title-row{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px}.notes-actions{display:flex;gap:6px}.inline-notes{width:100%;box-sizing:border-box;resize:vertical;min-height:72px;padding:9px 11px;border:1px solid #ddd6fe;border-radius:10px;background:#fff;color:inherit;font:inherit}.english-refresh-row{display:flex;gap:8px;align-items:center}.english-refresh-row input{flex:1}.english-refresh-btn{flex:0 0 auto}.nickname-random,.copy-user-id{border:0;background:transparent;cursor:pointer;font-size:1rem}.copy-user-id-status{color:#2b9b67;font-size:.75rem;font-weight:700}.folder-share-top{font-size:24px}.header-home-button{border:0;background:transparent;padding:0 5px 0 0;font:inherit;cursor:pointer;vertical-align:baseline}.theme-pink .profile-close,.theme-pink .back,.theme-pink .folder-share-top{background:#fff0f5!important;color:#d94f86!important;border-color:#ffd6e4!important}.theme-pink .avatar-upload{color:#d94f86!important;border-color:#ef9bbb!important}.theme-pink .stat strong,.theme-pink .stats strong,.theme-pink .bar span{color:#d94f86!important}.theme-pink .bar span{background:#f06f9d!important}
    /* 5.0.10 mobile/UI fixes */
    .back{background:transparent!important;border-color:transparent!important;box-shadow:none!important}
    .theme-pink .back{background:transparent!important;border-color:transparent!important}
    .folder-top-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px}.folder-home-top,.folder-share-top{background:transparent!important;border:0!important;padding:4px;font-size:24px}
    .topic-name-edit{position:absolute;right:12px;top:12px;border:0;background:transparent;padding:4px;font-size:1.05em;z-index:3}.sortable-topic{position:relative}.sortable-topic .topic-open{padding-right:48px!important}.section-title-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.tiny-add-btn{position:static!important;flex:0 0 auto;margin:0!important}
    .inline-notes{font-size:16px!important;line-height:1.4;touch-action:manipulation}
    .top{display:grid!important;grid-template-columns:minmax(82px,1fr) minmax(0,2fr) minmax(82px,1fr)!important;align-items:center}.top>h2{text-align:center!important;margin-left:0!important;margin-right:0!important;min-width:0}.top>:last-child{justify-self:end}.top>.back{justify-self:start}
    .scope-buttons{display:grid!important;grid-template-columns:1fr 1fr!important;gap:0!important;overflow:hidden}.scope-buttons .scope-btn{width:100%!important;min-width:0!important;border-radius:0!important}.scope-buttons .scope-btn:nth-child(1){border-right:1px solid rgba(112,75,228,.22)!important}.scope-buttons .scope-btn:nth-child(3){grid-column:1/-1;border-top:1px solid rgba(112,75,228,.22)!important}.theme-pink .scope-buttons .scope-btn:nth-child(1),.theme-pink .scope-buttons .scope-btn:nth-child(3){border-color:rgba(217,79,134,.22)!important}
    .match-grid{grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;gap:10px!important;width:100%;max-width:100%;overflow:hidden}.match-column{min-width:0}.match-card{min-width:0;max-width:100%;padding:10px 6px!important;font-size:clamp(15px,4vw,20px)!important;overflow-wrap:anywhere;word-break:break-word}
    body.is-home-view #headerHomeButton{display:none!important}.app>header{position:relative}.home .version-user-row{position:absolute!important;top:-98px!important;right:8px!important;z-index:20}.home .user-avatar-button{width:46px!important;height:46px!important}
...
async function loadFromSupabase(){
  ...
  try{
    ...
    const byOrder=(a,b)=>{const ao=Number(a.sort_order),bo=Number(b.sort_order);if(Number.isFinite(ao)&&Number.isFinite(bo)&&ao!==bo)return ao-bo;if(Number.isFinite(ao)!==Number.isFinite(bo))return Number.isFinite(ao)?-1:1;return 0};
    topics = (topicsResult.data || []).map(({user_id,...item})=>item).sort(byOrder);
    school = (schoolResult.data || []).map(({user_id,...x}) => ({...x,parentId:x.parentId??null})).sort(byOrder);
    phrases = (phrasesResult.data || []).map(({user_id,...item})=>item).sort(byOrder);
...
function topicForCloud(t){
  return {
    id: t.id,
    name: t.name,
    sort_order: Number.isFinite(Number(t.sort_order)) ? Number(t.sort_order) : topics.indexOf(t),
...
function schoolForCloud(item){
  return {
    ...
    sort_order: Number.isFinite(Number(item.sort_order)) ? Number(item.sort_order) : school.indexOf(item),
...
function phraseForCloud(item){
  return {
    id: item.id,
    name: item.name,
    sort_order: Number.isFinite(Number(item.sort_order)) ? Number(item.sort_order) : phrases.indexOf(item),
    shared_from_nickname: item.shared_from_nickname || null,
    shared_from_user_id: item.shared_from_user_id || null,
    user_id: currentUserId
  };
...
function normalizeFolderOrders(){topics.forEach((x,i)=>x.sort_order=i);school.forEach((x,i)=>x.sort_order=i);phrases.forEach((x,i)=>x.sort_order=i)}
function save(){
  if(!currentUserId)return;
...
function updateStaticAppVersion(){
  const target='PRO 5.0.10 · 23.09.2026';
...
function shell(title,body,back){const right=state.view==='detail'?`<div class="folder-top-actions"><button class="home-mini folder-home-top" data-act="home" type="button" aria-label="На головну" title="На головну">🏠</button><button class="home-mini folder-share-top" data-act="share-folder" type="button" aria-label="Поділитися папкою" title="Поділитися папкою">↔</button></div>`:'<span></span>';return `<section class="panel"><div class="top">${back?`<button class="back" data-act="${back}">← Назад</button>`:'<span></span>'}<h2>${esc(title)}</h2>${right}</div>${body}</section>`}
function render(){try{ensureThemeStyles();applyAppTheme();document.body.classList.toggle('is-home-view',state.view==='home');app.innerHTML=(views[state.view]||views.home)();bind();setupScrollUi();setupFolderSwipeBack();updateStaticAppVersion();updateAuthUI().then(refreshAvatarButton).catch(console.error);if(state.view==='study'&&state.mode==='speaking')setTimeout(startSpeaking,250)}catch(e){console.error(e);app.innerHTML=`<section class="panel"><h2>Сталася помилка</h2><p>${esc(e.message)}</p><button class="btn primary" data-act="home">На головну</button></section>`;bind()}}
const views={
  home:()=>`
  <section class="panel">
    <div class="home">
      ...
      <div class="app-footer-version">PRO 5.0.10 · 23.09.2026</div>
    </div>
  </section>
`,
...
function collectionView(c,title){
  ...
  return shell(heading,`<div class="actions"><button class="btn primary" data-act="new-topic" data-c="${c}">+ ${c==='school'?'Папка / урок':c==='phrases'?'Розділ':'Тема'}</button><button class="btn secondary" data-act="photo-import-collection" data-c="${c}">📷 Додати з фото</button></div><div class="sort-hint">Перетягни за ☰ або використовуй ↑ ↓.</div><div class="list sortable-list" data-c="${c}">${items||'<div class="empty">Поки порожньо.</div>'}</div>`,back)
}
function detailView(){
...
function wordList(arr){
  ...
  return `<div class="list">${arr.map(w=>{
    ...
    const duplicateInFolder=words.some(x=>x.id!==w.id&&x.collection===w.collection&&x.topicId===w.topicId&&x.korean.trim()===w.korean.trim());
    const extras=[];
    ...
    extras.push(`<div class="expanded-notes-editor" style="margin-top:10px">
      <div class="notes-title-row"><b>📝 Нотатки</b><button class="smallbtn" data-act="speak-note" data-id="${esc(w.id)}" type="button" title="Прослухати нотатку">🔊</button></div>
      <textarea class="inline-notes" data-word-id="${esc(w.id)}" rows="3" placeholder="Додати нотатку…">${esc(w.notes||'')}</textarea>
    </div>`);
    ...
    return `<div class="word ${expanded?'expanded':''} ${selected?'bulk-selected':''}" data-word-id="${esc(w.id)}">
      ...
      ${w.notes&&!expanded?`<div class="notes-preview" style="margin-top:10px">
      ...
      ${duplicateInFolder?'<div class="duplicate-badge">Дублікат</div>':''}
...
function bind(){app.onclick=handleClick;document.querySelectorAll('.inline-notes').forEach(field=>{['click','pointerdown','keydown'].forEach(type=>field.addEventListener(type,e=>e.stopPropagation()));let timer=null;field.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>{const w=words.find(x=>x.id===field.dataset.wordId);if(w){w.notes=field.value;save()}},250)});field.addEventListener('blur',()=>{const w=words.find(x=>x.id===field.dataset.wordId);if(w){w.notes=field.value;save()}})}); const headerHome=document.getElementById('headerHomeButton');if(headerHome&&!headerHome.dataset.bound){headerHome.dataset.bound='1';headerHome.addEventListener('click',()=>{state.bulkMove=false;state.selectedWordIds=[];state.expandedWordId=null;setView('home')})} const loginForm = document.getElementById('loginForm');
...
function setupFolderSwipeBack(){
  if(state.view!=='detail'||window.__folderSwipeBound)return;
  window.__folderSwipeBound=true;let sx=0,sy=0,tracking=false;
  const down=e=>{if(state.view!=='detail'||e.clientX>32||e.target.closest('input,textarea,select,button'))return;sx=e.clientX;sy=e.clientY;tracking=true};
  const up=e=>{if(!tracking)return;tracking=false;const dx=e.clientX-sx,dy=Math.abs(e.clientY-sy);if(dx>75&&dy<55){const isSchool=state.collection==='school';const parent=isSchool&&school.find(x=>x.id===state.topicId)?.parentId;if(parent){state.topicId=parent;setView('detail')}else setView(state.collection==='school'?'school':state.collection==='phrases'?'phrases':state.collection)}};
...
function handleClick(e){if(e.target.closest('.inline-notes'))return;const flash=e.target.closest('#flash');if(flash){flash.classList.toggle('flipped');return}const b=e.target.closest('button');const card=e.target.closest('.word[data-word-id]');if(state.bulkMove&&card){const id=card.dataset.wordId;state.selectedWordIds=state.selectedWordIds.includes(id)?state.selectedWordIds.filter(x=>x!==id):[...state.selectedWordIds,id];render();return}if(!b){if(card){const id=card.dataset.wordId;state.expandedWordId=state.expandedWordId===id?null:id;render()}return}const a=b.dataset.act;
  ...
  else if(a==='share-folder'){openShareFolderModal()} else if(a==='edit-note-inline'){startInlineNoteEdit(b.dataset.id)}
  else if(a==='set-theme'){setCloudThemeValue(b.dataset.theme==='pink'?'pink':'purple').then(()=>render()).catch(error=>{console.error(error);alert('Не вдалося зберегти тему в профілі.')})}
...
function moveTopic(c,id,dir){const visible=c==='school'?schoolChildren(school.find(t=>t.id===id)?.parentId):listFor(c),i=visible.findIndex(t=>t.id===id),j=dir==='up'?i-1:i+1;if(i<0||j<0||j>=visible.length)return;const backing=c==='school'?school:(c==='phrases'?phrases:topics),a=backing.findIndex(t=>t.id===visible[i].id),b=backing.findIndex(t=>t.id===visible[j].id);if(a<0||b<0)return;[backing[a],backing[b]]=[backing[b],backing[a]];save();render()}
...
function setupTopicSorting(){
  document.querySelectorAll('.sortable-topic').forEach(el=>{
    ...
    el.addEventListener('drop',e=>{
      ...
      const backing=c==='phrases'?phrases:topics,from=backing.findIndex(t=>t.id===sourceId),to=backing.findIndex(t=>t.id===targetId);
      if(from<0||to<0)return;const [moved]=backing.splice(from,1);backing.splice(to,0,moved);save();render();
    });
  });
...
function startMode(mode){stopSpeakingRecognition();state.mode=mode;const shuffled=shuffle(state.studySource);state.study=state.studyLimit==='20'?shuffled.slice(0,20):state.studyLimit==='learning'?shuffled.filter(w=>wordStatusKey(w)!=='learned'):shuffled;if(!state.study.length){alert('У цьому наборі немає нових або знайомих слів.');return}state.index=0;if(mode==='match'){state.matchOffset=0;state.matchSelected=null;state.matchMatched=[];state.matchWrong=[];state.matchMistakeWordIds=[];prepareMatchRound()}setView('study')}
...
async function translateToEnglish(ko){const existing=words.find(w=>w.korean.trim()===ko&&w.english?.trim());if(existing)return existing.english.trim();const seed=(window.SEED_WORDS||[]).find(w=>w.korean?.trim()===ko&&w.english?.trim());if(seed)return seed.english.trim();const basics={'나':'I / me (informal)','저':'I / me (polite)','너':'you (informal)','우리':'we / our','학교':'school','배우다':'to learn','선생님':'teacher'};if(basics[ko])return basics[ko];try{const url=`https://translate.googleapis.com/translate_a/single?client=gtx&sl=ko&tl=en&dt=t&q=${encodeURIComponent(ko)}`;const r=await fetch(url,{headers:{Accept:'application/json'}});if(r.ok){const data=await r.json();const translated=(data?.[0]||[]).map(x=>x?.[0]||'').join('').trim();if(translated&&translated.toLowerCase()!==ko.toLowerCase())return translated}}catch(e){console.warn('English translation failed',e)}return ''}
...
async function refreshEnglishTranslation(){const ko=$('#ko')?.value.trim(),en=$('#en');if(!ko||!en){alert('Спочатку введи корейське слово.');return}const btn=document.querySelector('.english-refresh-btn');if(btn){btn.disabled=true;btn.textContent='…'}try{const translated=await translateToEnglish(ko);if(translated)en.value=translated;else alert('Переклад не знайдено.')}finally{if(btn){btn.disabled=false;btn.textContent='🔄'}}}
function startInlineNoteEdit(id){const card=document.querySelector(`.word[data-word-id="${CSS.escape(id)}"]`);const box=card?.querySelector('.expanded-notes-editor');const w=words.find(x=>x.id===id);if(!box||!w)return;box.innerHTML=`<div class="notes-title-row"><b>📝 Нотатки</b><button class="smallbtn note-save-inline" type="button">✓ Готово</button></div><textarea class="inline-notes" data-word-id="${esc(w.id)}" rows="3" placeholder="Додати нотатку…">${esc(w.notes||'')}</textarea>`;const field=box.querySelector('.inline-notes'),done=box.querySelector('.note-save-inline');const saveNote=()=>{const next=field.value.trim();w.notes=next;save();render()};['click','pointerdown','keydown'].forEach(type=>field.addEventListener(type,e=>e.stopPropagation()));done.addEventListener('click',e=>{e.stopPropagation();saveNote()});field.focus()}
...
function koreanNumberForms(n){
  n=Number(n);if(!Number.isInteger(n)||n<0||n>9999)return [];
  const forms=new Set();
  const sinoDigits=['','일','이','삼','사','오','육','칠','팔','구'];
  if(n===0)forms.add('영');
  else{let x=n,out='';for(const [v,u] of [[1000,'천'],[100,'백'],[10,'십']]){const d=Math.floor(x/v);if(d){out+=(d===1?'':sinoDigits[d])+u;x%=v}}if(x)out+=sinoDigits[x];forms.add(out)}
  const ones=['','하나','둘','셋','넷','다섯','여섯','일곱','여덟','아홉'];
  const tens=['','','스물','서른','마흔','쉰','예순','일흔','여든','아흔'];
  if(n>=1&&n<=99){const t=Math.floor(n/10),o=n%10;forms.add((t?tens[t]:'')+(o?ones[o]:''))}
  return [...forms];
}
function speakingVariant(value,expected){
  const clean=normalizeAnswer(value);if(/^\d+$/.test(clean)){const n=Number(clean);const forms=koreanNumberForms(n);if(forms.some(f=>normalizeAnswer(f)===normalizeAnswer(expected)))return expected}
  return value;
}
async function startSpeaking(){
  ...
  rec.onresult=e=>{
    ...
    const heardRaw=[...last].map(x=>x.transcript.trim()).filter(Boolean);
    const heard=heardRaw.map(x=>speakingVariant(x,w.korean));
...
async function forceUpdate(){
  ...
  try{
    ...
    const assets=['./','./index.html','./style.css','./app.js','./data.js','./sw.js','./login.html','./login-style.css','./login.js','./manifest.webmanifest'];
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=5.0.10',{updateViaCache:'none'}).catch(console.error))}
