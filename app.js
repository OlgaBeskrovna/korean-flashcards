...
function scoreOcrText(t){
  ...
  const letters=(t.match(/[A-Za-zА-Яа-яІіЇїЄє]/g)||[]).length;
...
async function recognizeOcrImage(image,progress,label,psm='6'){
...
function extractKoreanWordsFromOcr(result){
  const data=result?.data||{};
  const lines=data.lines||[];
  const found=[];

  // Prefer OCR line geometry: this keeps two-column tables from being merged.
  for(const line of lines){
    const raw=String(line.text||'').trim();
    if(!raw)continue;
    const parts=raw.match(/[\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]+/g)||[];
    for(const part of parts){
      const word=part.trim();
      if(word.length>=1 && /[\uAC00-\uD7AF]/u.test(word))found.push(word);
    }
  }

  // Fallback for Tesseract builds that don't expose data.lines.
  if(!found.length){
    const raw=String(data.text||'');
    for(const part of raw.match(/[\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]+/g)||[]){
      if(/[\uAC00-\uD7AF]/u.test(part))found.push(part);
    }
  }

  // Preserve visual order and remove duplicates.
...
async function runOcr(){
  ...
  try{
    ...
    // PSM 11 ("sparse text") works much better for photographed tables/two columns.
    const sparse=await recognizeOcrImage(enhanced,progress,'Розпізнавання таблиці',11);
    let words=extractKoreanWordsFromOcr(sparse);

    // Retry as a text block if sparse OCR found suspiciously little.
    if(words.length<5){
      const block=await recognizeOcrImage(enhanced,progress,'Додаткова перевірка',6);
      const blockWords=extractKoreanWordsFromOcr(block);
      if(blockWords.length>words.length)words=blockWords;
    }

    if(words.length){
      verified.value=words.map((w,i)=>`${i+1}. ${w} — `).join('\n');
      if(status){
        status.hidden=false;
        status.textContent=`Знайдено корейських записів: ${words.length}. Додай переклад або відредагуй список перед додаванням.`;
      }
      progress.textContent='Готово. Перевір розпізнаний список нижче.';
      return;
    }

    // Last fallback: preserve raw OCR instead of inventing broken separators.
    const raw=String(sparse?.data?.text||'').trim();
    verified.value=raw;
    if(status){
      status.hidden=false;
      status.textContent='Не вдалося надійно виділити корейські слова. Перевір текст вручну.';
    }
    progress.textContent='Розпізнавання завершено.';
  }catch(error){
    console.error(error);
    progress.textContent='Не вдалося розпізнати зображення.';
    alert('Помилка розпізнавання: '+error.message);
  }
}
