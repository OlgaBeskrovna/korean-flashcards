...
function selectedAvatar(){
  ...
  return selected==='custom'&&custom?custom:`./${DEFAULT_AVATARS.includes(selected)?selected:'avatar-1.png'}?v=5.0.0`;
}
...
async function editNickname(){
  ...
  const {data,error}=await supabaseClient.rpc('set_nickname',{new_nickname:value});
  if(error){alert(error.message.includes('unique')?'Цей нікнейм уже зайнятий.':error.message);return}
  ...
  modal.innerHTML=`<div class="profile-modal" role="dialog" aria-modal="true" aria-label="Профіль">
    ...
    <div class="avatar-grid">
      ${DEFAULT_AVATARS.map((name,i)=>`<button class="avatar-choice ${selected===name?'selected':''}" data-avatar="${name}" type="button" aria-label="Аватар ${i+1}">
        <img src="./${name}?v=5.0.0" alt="Аватар ${i+1}">
    ...
    <div class="profile-version">PRO 5.0.0 · 29.08.2026</div>
  </div>`;
    ...
    <div class="theme-picker">
      ...
      <div class="theme-options">
        <button class="theme-card ${theme==='purple'?'selected':''}" data-act="set-theme" data-theme="purple" type="button">
          <img src="./icon-purple.png?v=5.0.0" alt="">
        ...
        <button class="theme-card pink ${theme==='pink'?'selected':''}" data-act="set-theme" data-theme="pink" type="button">
          <img src="./icon-pink.png?v=5.0.0" alt="">
...
function updateStaticAppVersion(){
  const target='PRO 5.0.0 · 29.08.2026';
...
const views={
  ...
  <section class="panel">
    <div class="home">
      ...
      <div class="app-footer-version">PRO 5.0.0 · 29.08.2026</div>
    </div>
  </section>
`,
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=5.0.0',{updateViaCache:'none'}).catch(console.error))}
