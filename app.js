...
const views={
  ...
  <section class="panel">
    <div class="home">
      <div class="version-user-row" style="display:flex;align-items:center;justify-content:center;gap:10px;position:relative;margin-bottom:18px">
        <div style="display:inline-flex;align-items:center;padding:6px 12px;border-radius:999px;background:#eee9ff;color:#694ae2;font-weight:800">PRO 4.9.3 · 29.08.2026</div>
        ...
        >
          ...
          <button class="btn secondary" data-act="change-password" type="button" style="width:100%;margin-bottom:8px">🔐 Змінити пароль</button>
      ...
      <div id="authBox" class="notice">
        ...
        <form id="loginForm" class="form" style="display:none; margin-top:12px">
          ...
          <button class="btn secondary" data-act="forgot-password" type="button">
            Забула пароль
          </button>
        </form>
      </div>
...
function handleClick(e){if(e.target.closest('.inline-notes'))return;const flash=e.target.closest('#flash');if(flash){flash.classList.toggle('flipped');return}const b=e.target.closest('button');const card=e.target.closest('.word[data-word-id]');if(state.bulkMove&&card){const id=card.dataset.wordId;state.selectedWordIds=state.selectedWordIds.includes(id)?state.selectedWordIds.filter(x=>x!==id):[...state.selectedWordIds,id];render();return}if(!b){if(card){const id=card.dataset.wordId;state.expandedWordId=state.expandedWordId===id?null:id;render()}return}const a=b.dataset.act;
  ...
  else if(a==='change-password'){toggleUserMenu(false);openPasswordModal()}
  else if(a==='forgot-password'){
    const input=document.getElementById('loginEmail');
    const email=input?.value.trim()||prompt('Введи email для відновлення пароля:')||'';
    if(!email)return;
    requestPasswordReset(email).then(()=>{
      alert('Лист для зміни пароля надіслано. Відкрий посилання з листа на цьому телефоні або комп’ютері.');
    }).catch(error=>{
      console.error(error);
...
async function forceUpdate(){
  ...
  try{
    ...
    const assets=['./','./index.html','./style.css','./app.js','./data.js','./sw.js','./manifest.webmanifest'];
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=4.9.3',{updateViaCache:'none'}).catch(console.error))}
