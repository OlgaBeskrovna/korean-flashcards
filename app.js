...
function selectedAvatar(){
  ...
  return selected==='custom'&&custom?custom:`./${DEFAULT_AVATARS.includes(selected)?selected:'avatar-1.png'}?v=5.0.2`;
}
...
function socialAvatar(profile){
  if(profile?.avatar_value?.startsWith('data:image/'))return profile.avatar_value;
  const name=DEFAULT_AVATARS.includes(profile?.avatar_value)?profile.avatar_value:'avatar-1.png';
  return `./${name}?v=5.0.2`;
}
async function loadSocialData(){
  if(!currentUserId)return;
  try{
    const {error:ensureError}=await supabaseClient.rpc('ensure_my_profile_v2');
...
async function syncAvatarToProfile(){
  if(!currentUserId)return;
  const selected=localStorage.getItem(avatarStorageKey())||'avatar-1.png';
  const value=selected==='custom'?(localStorage.getItem(customAvatarStorageKey())||'avatar-1.png'):selected;
  const {error}=await supabaseClient.from('profiles').update({avatar_value:value}).eq('user_id',currentUserId);
  if(error)console.warn('Avatar cloud sync failed',error); else if(socialProfile)socialProfile.avatar_value=value;
}
async function editNickname(){
  const old=socialProfile?.nickname||''; const value=prompt('Новий унікальний нікнейм:',old)?.trim();
  if(!value||value===old)return;
  const {data,error}=await supabaseClient.rpc('set_nickname_v2',{p_nickname:value});
  if(error){alert(error.message.includes('зайнятий')||error.code==='23505'?'Цей нікнейм уже зайнятий.':error.message);return}
  if(socialProfile) socialProfile.nickname=String(data||value);
  await loadSocialData();
  if(!socialProfile?.nickname){alert('Не вдалося перечитати нікнейм із профілю. Спробуйте ще раз.');return}
  openProfileModal();
}
  ...
  modal.innerHTML=`<div class="profile-modal" role="dialog" aria-modal="true" aria-label="Профіль">
    ...
    <div class="avatar-grid">
      ${DEFAULT_AVATARS.map((name,i)=>`<button class="avatar-choice ${selected===name?'selected':''}" data-avatar="${name}" type="button" aria-label="Аватар ${i+1}">
        <img src="./${name}?v=5.0.2" alt="Аватар ${i+1}">
    ...
    <div class="profile-version">PRO 5.0.2 · 29.08.2026</div>
  </div>`;
    ...
    <div class="theme-picker">
      ...
      <div class="theme-options">
        <button class="theme-card ${theme==='purple'?'selected':''}" data-act="set-theme" data-theme="purple" type="button">
          <img src="./icon-purple.png?v=5.0.2" alt="">
        ...
        <button class="theme-card pink ${theme==='pink'?'selected':''}" data-act="set-theme" data-theme="pink" type="button">
          <img src="./icon-pink.png?v=5.0.2" alt="">
...
function updateStaticAppVersion(){
  const target='PRO 5.0.2 · 29.08.2026';
...
const views={
  ...
  <section class="panel">
    <div class="home">
      ...
      <div class="app-footer-version">PRO 5.0.2 · 29.08.2026</div>
    </div>
  </section>
`,
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=5.0.2',{updateViaCache:'none'}).catch(console.error))}
