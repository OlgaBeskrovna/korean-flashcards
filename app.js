...
const KEYS={topics:'kf3_topics',school:'kf3_school',phrases:'kf3_phrases',themes:'kf3_themes',lingory:'kf3_lingory',airlearn:'kf3_airlearn',words:'kf3_words'};
let currentUserId=null;
function userLocalKey(key){
  return currentUserId ? `${key}_${currentUserId}` : `${key}_guest`;
}
const app=document.getElementById('app');
let topics=[],school=[],phrases=[],themes=[],lingory=[],airlearn=[],words=[];
...
function selectedAvatar(){
  ...
  return selected==='custom'&&custom?custom:`./${DEFAULT_AVATARS.includes(selected)?selected:'avatar-1.png'}`;
}
  ...
  modal.innerHTML=`<div class="profile-modal" role="dialog" aria-modal="true" aria-label="Профіль">
    ...
    <div class="avatar-grid">
      ${DEFAULT_AVATARS.map((name,i)=>`<button class="avatar-choice ${selected===name?'selected':''}" data-avatar="${name}" type="button" aria-label="Аватар ${i+1}">
        <img src="./${name}?v=4.9.9" alt="Аватар ${i+1}">
    ...
    <p class="muted avatar-private">Аватарка буде видно лише вам і не впливає на дані</p>
    <button class="btn secondary profile-logout" type="button">⇥ Вийти з акаунта</button>
    <div class="profile-version">PRO 4.9.9 · 29.08.2026</div>
  </div>`;
    ...
    <div class="theme-picker">
      ...
      <div class="theme-options">
        <button class="theme-card ${theme==='purple'?'selected':''}" data-act="set-theme" data-theme="purple" type="button">
          <img src="./icon-purple.png?v=4.9.9" alt="">
        ...
        <button class="theme-card pink ${theme==='pink'?'selected':''}" data-act="set-theme" data-theme="pink" type="button">
          <img src="./icon-pink.png?v=4.9.9" alt="">
...
function ensureThemeStyles(){
  ...
  style.textContent=`
    ...
    .profile-email{margin-top:3px;color:#7d7690;font-size:.92rem;word-break:break-word}
    ...
    .avatar-choice{padding:0!important;border:3px solid #eee7ff!important;overflow:hidden!important}.avatar-choice.selected{border:4px solid #704be4!important;padding:0!important}.avatar-choice img{position:absolute;inset:0;width:100%!important;height:100%!important;object-fit:cover!important}.avatar-check{z-index:2;right:1px;bottom:5px}
    body.theme-pink .avatar-choice.selected{border-color:#df6f9b!important} body.theme-pink .avatar-check{background:#df6f9b!important}
    ...
    .match-help{text-align:center;color:#7c748e;margin:4px 0 18px}.match-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.match-card{min-height:92px;border:2px solid #e7e1f2;border-radius:20px;background:#fff;color:#29233c;padding:12px 8px;font-size:clamp(16px,4.3vw,22px);font-weight:750;line-height:1.25;transition:.15s}.match-card.selected{background:#eee8ff;border-color:#704be4;color:#5e3ed0}.theme-pink .match-card.selected{background:#fff0f5;border-color:#df6f9b;color:#c94f7e}.match-card.matched{background:#e6f8ef!important;border-color:#27b878!important;color:#148457!important}.match-card.wrong{background:#ffe9ea!important;border-color:#e6535c!important;color:#bd3139!important}.match-card:disabled{opacity:1}.more-menu{display:grid;gap:14px}
...
function load(){
  if(!currentUserId){
    topics=[]; school=[]; phrases=[]; themes=[]; lingory=[]; airlearn=[]; words=[];
    return;
  }
  ...
  try{themes=JSON.parse(localStorage.getItem(userLocalKey(KEYS.themes)))||[]}catch{themes=[]}
  try{lingory=JSON.parse(localStorage.getItem(userLocalKey(KEYS.lingory)))||[]}catch{lingory=[]}
  try{airlearn=JSON.parse(localStorage.getItem(userLocalKey(KEYS.airlearn)))||[]}catch{airlearn=[]}
...
async function loadFromSupabase(){
  ...
  try{
    const [
      ...
      supabaseClient.from('themes').select('*').eq('user_id',currentUserId),
      supabaseClient.from('lingory').select('*').eq('user_id',currentUserId),
      supabaseClient.from('airlearn').select('*').eq('user_id',currentUserId),
    ...
    themes = (themesResult.data || []).map(({user_id,...item})=>item);
    lingory = (lingoryResult.data || []).map(({user_id,...item})=>item);
    airlearn = (airlearnResult.data || []).map(({user_id,...item})=>item);
    ...
    localStorage.setItem(userLocalKey(KEYS.themes), JSON.stringify(themes));
    localStorage.setItem(userLocalKey(KEYS.lingory), JSON.stringify(lingory));
    localStorage.setItem(userLocalKey(KEYS.airlearn), JSON.stringify(airlearn));
    localStorage.setItem(userLocalKey(KEYS.words), JSON.stringify(words));

    console.log('Supabase user data loaded:', currentUserId, topics.length, school.length, phrases.length, themes.length, lingory.length, airlearn.length, words.length);
    return true;
  }catch(error){
...
/* =========================
   THEMES / LINGORY / AIRLEARN
========================= */
const extraCloudState={
  themes:{last:new Map(),timer:null,saving:false},
  lingory:{last:new Map(),timer:null,saving:false},
  airlearn:{last:new Map(),timer:null,saving:false}
};
function extraArray(name){return name==='themes'?themes:name==='lingory'?lingory:airlearn}
function extraForCloud(item){return {id:item.id,name:item.name,user_id:currentUserId}}
function extraSignature(item){return JSON.stringify(extraForCloud(item))}
function initExtraCollectionSnapshots(){
  for(const name of ['themes','lingory','airlearn']) extraCloudState[name].last=new Map(extraArray(name).map(x=>[x.id,extraSignature(x)]));
}
async function syncExtraCollection(name){
  if(!currentUserId)return;
  const st=extraCloudState[name]; if(st.saving)return; st.saving=true;
  try{
    const arr=extraArray(name),currentIds=new Set(arr.map(x=>x.id)),changed=[];
    for(const item of arr)if(st.last.get(item.id)!==extraSignature(item))changed.push(extraForCloud(item));
    const deleted=[...st.last.keys()].filter(id=>!currentIds.has(id));
    if(changed.length){const {error}=await supabaseClient.from(name).upsert(changed,{onConflict:'id'});if(error)throw error}
    if(deleted.length){const {error}=await supabaseClient.from(name).delete().in('id',deleted).eq('user_id',currentUserId);if(error)throw error}
    st.last=new Map(arr.map(x=>[x.id,extraSignature(x)]));
  }catch(error){console.error(`Supabase ${name} save failed:`,error)}finally{st.saving=false}
}
function scheduleExtraCloudSave(name){const st=extraCloudState[name];clearTimeout(st.timer);st.timer=setTimeout(()=>syncExtraCollection(name),300)}
...
function save(){
  ...
  scheduleExtraCloudSave('themes');
  scheduleExtraCloudSave('lingory');
  scheduleExtraCloudSave('airlearn');
  scheduleWordsCloudSave();
}

function listFor(c){
  if(c==='school')return school; if(c==='phrases')return phrases;
  if(c==='themes')return themes; if(c==='lingory')return lingory; if(c==='airlearn')return airlearn;
  return topics;
}
...
function updateStaticAppVersion(){
  const target='PRO 4.9.9 · 29.08.2026';
...
function prepareMatchRound(){
  ...
  state.matchCards=shuffle(source.flatMap(w=>[{key:w.id+':ko',wordId:w.id,side:'ko',text:w.korean},{key:w.id+':uk',wordId:w.id,side:'uk',text:w.ukrainian}]));
}
function matchStudyView(){
  ...
  return shell(`Знайди пари · ${roundNo} / ${total}`,`<div class="match-help">Знайди 6 пар: 🇰🇷 корейське слово + 🇺🇦 український переклад</div><div class="match-grid">${state.matchCards.map(c=>{const matched=state.matchMatched.includes(c.key),wrong=state.matchWrong.includes(c.key),selected=state.matchSelected===c.key;return `<button type="button" class="match-card ${matched?'matched':''} ${wrong?'wrong':''} ${selected?'selected':''}" data-act="match-card" data-key="${esc(c.key)}" ${matched?'disabled':''}>${esc(c.text)}</button>`}).join('')}</div>`,'back-to-modes');
}
function handleMatchCard(key){
  if(state.matchBusy||state.matchMatched.includes(key))return; const card=state.matchCards.find(c=>c.key===key); if(!card)return;
  ...
  const firstKey=state.matchSelected; const ok=first.wordId===card.wordId&&first.side!==card.side; state.matchBusy=true;
  if(ok){state.matchMatched.push(firstKey,key);state.matchSelected=null;state.matchBusy=false;const w=state.matchRound.find(w=>w.id===card.wordId);mark(w,true);render();if(state.matchMatched.length===state.matchCards.length)setTimeout(()=>{state.matchOffset=(state.matchOffset||0)+6;if(state.matchOffset>=state.study.length){state.matchBusy=false;alert('🎉 Усі пари знайдено!');setView('modes')}else{prepareMatchRound();state.matchBusy=false;render()}},650)}
  else{state.matchWrong=[firstKey,key];render();setTimeout(()=>{state.matchWrong=[];state.matchSelected=null;state.matchBusy=false;render()},700)}
}
...
function startMode(mode){stopSpeakingRecognition();state.mode=mode;const shuffled=shuffle(state.studySource);state.study=state.studyLimit==='20'?shuffled.slice(0,20):shuffled;state.index=0;if(mode==='match'){state.matchOffset=0;state.matchSelected=null;state.matchMatched=[];state.matchWrong=[];prepareMatchRound()}setView('study')}
function mark(w,ok){
  if(!w)return;
...
function saveTopic(e){e.preventDefault();const n=$('#topicName').value.trim();if(!n)return;const list=listFor(state.collection),t=list.find(x=>x.id===state.editTopicId);if(t)t.name=n;else{const item={id:uid(),name:n,...(state.collection==='school'?{parentId:state.schoolParentId??null}:{})};if(state.collection==='school')school.push(item);else if(state.collection==='phrases')phrases.push(item);else if(state.collection==='themes')themes.push(item);else if(state.collection==='lingory')lingory.push(item);else if(state.collection==='airlearn')airlearn.push(item);else topics.push(item)}state.editTopicId=null;save();if(state.collection==='school'&&state.schoolParentId&&state.returnView==='detail'){state.topicId=state.returnTopicId||state.schoolParentId;setView('detail')}else setView(collectionViewName(state.collection))}
...
function exportJson(){const data={format:'korean-flashcards-pro',version:'4.9.9',exportedAt:new Date().toISOString(),topics,school,phrases,themes,lingory,airlearn,words};const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`korean-flashcards-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url)}
async function importJson(e){const file=e.target.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text());if(!Array.isArray(data.topics)||!Array.isArray(data.school)||!Array.isArray(data.phrases)||!Array.isArray(data.words))throw new Error('Неправильний формат файлу');if(!confirm('Замінити поточні дані даними з резервної копії?'))return;topics=data.topics;school=data.school;phrases=data.phrases;themes=Array.isArray(data.themes)?data.themes:[];lingory=Array.isArray(data.lingory)?data.lingory:[];airlearn=Array.isArray(data.airlearn)?data.airlearn:[];words=data.words.map(normalizeWord);save();alert('Дані успішно імпортовано.');setView('home')}catch(err){alert('Не вдалося імпортувати файл: '+err.message)}finally{e.target.value=''}}
...
async function forceUpdate(){
  ...
  try{
    ...
    const assets=['./','./index.html','./style.css','./app.js','./data.js','./sw.js','./login.html','./login-style.css','./login.js','./login-korea.png','./login-purple-bg.jpg','./icon-purple.png','./icon-pink.png','./manifest.webmanifest'];
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=4.9.9',{updateViaCache:'none'}).catch(console.error))}
