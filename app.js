  ...
  modal.innerHTML=`<div class="profile-modal" role="dialog" aria-modal="true" aria-label="Профіль">
    ...
    <div class="avatar-grid">
      ${DEFAULT_AVATARS.map((name,i)=>`<button class="avatar-choice ${selected===name?'selected':''}" data-avatar="${name}" type="button" aria-label="Аватар ${i+1}">
        <img src="./${name}?v=4.9.10" alt="Аватар ${i+1}">
    ...
    <div class="profile-version">PRO 4.9.10 · 29.08.2026</div>
  </div>`;
    ...
    <div class="theme-picker">
      ...
      <div class="theme-options">
        <button class="theme-card ${theme==='purple'?'selected':''}" data-act="set-theme" data-theme="purple" type="button">
          <img src="./icon-purple.png?v=4.9.10" alt="">
        ...
        <button class="theme-card pink ${theme==='pink'?'selected':''}" data-act="set-theme" data-theme="pink" type="button">
          <img src="./icon-pink.png?v=4.9.10" alt="">
...
function updateStaticAppVersion(){
  const target='PRO 4.9.10 · 29.08.2026';
...
function handleMatchCard(key){
  ...
  const koreanCard=first.side==='ko'?first:card;
  const w=state.matchRound.find(x=>x.id===koreanCard.wordId);
  if(ok){
    state.matchMatched.push(firstKey,key); state.matchSelected=null; state.matchBusy=false; mark(w,true); render();
    ...
    // Use the same progress/mistake mechanism as every other training mode.
    mark(w,false); state.matchWrong=[firstKey,key]; render();
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=4.9.10',{updateViaCache:'none'}).catch(console.error))}
