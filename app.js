...
function refreshAvatarButton(){
  ...
  if(!btn)return;
  btn.innerHTML=`<img src="${selectedAvatar()}" alt="Аватарка" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
}
function openProfileModal(){
  ...
  modal.innerHTML=`<div class="profile-modal">
    <button class="profile-close" type="button" data-act="close-profile">×</button>
    <h2>Профіль</h2>
    <h3>Аватарка</h3>
    <p class="muted profile-sub">Оберіть аватарку або завантажте свою</p>
    <div class="avatar-grid">
      ${DEFAULT_AVATARS.map((name,i)=>`<button class="avatar-choice ${selected===name?'selected':''}" data-act="select-avatar" data-avatar="${name}" type="button"><img src="./${name}?v=4.9.6" alt="Аватар ${i+1}">${selected===name?'<span class="avatar-check">✓</span>':''}</button>`).join('')}
    </div>
    ...
    ${custom&&selected==='custom'?`<div class="custom-avatar-preview"><img src="${custom}" alt="Моя аватарка"></div>`:''}
    <p class="muted avatar-private">Аватарка буде видно лише вам і не впливає на дані</p>
    <button class="btn secondary profile-logout" data-act="logout" type="button">⇥ Вийти з акаунта</button>
    <div class="profile-version">PRO 4.9.6 · 29.08.2026</div>
  </div>`;
  ...
  input?.addEventListener('change',e=>{
    ...
    reader.onload=()=>{
      ...
      modal.remove();refreshAvatarButton();openProfileModal();
    };
    ...
    <div class="theme-picker">
      ...
      <div class="theme-options">
        <button class="theme-card ${theme==='purple'?'selected':''}" data-act="set-theme" data-theme="purple" type="button">
          <img src="./icon-purple.png?v=4.9.6" alt="">
        ...
        <button class="theme-card pink ${theme==='pink'?'selected':''}" data-act="set-theme" data-theme="pink" type="button">
          <img src="./icon-pink.png?v=4.9.6" alt="">
...
function ensureThemeStyles(){
  if(document.getElementById('theme496Styles'))return;
  const style=document.createElement('style');
  style.id='theme496Styles';
  style.textContent=`
    .panel{position:relative}
    .version-user-row{position:absolute!important;top:-98px;right:18px;margin:0!important;z-index:20}
    .user-avatar-button{width:52px!important;height:52px!important;padding:2px!important;overflow:hidden;background:#eee9ff!important;border:2px solid #d8c9ff!important}
    ...
    .profile-close{position:absolute;right:22px;top:20px;width:48px;height:48px;border-radius:50%;border:1px solid #d9ceff;background:#f2edff;color:#6d4be2;font-size:30px}
    .profile-modal h2{margin:8px 0 28px}.profile-modal h3{font-size:22px;margin:0 0 5px}.profile-sub{margin:0 0 24px}
    .avatar-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;max-width:460px;margin:0 auto 24px}
    .avatar-choice{position:relative;aspect-ratio:1;border-radius:50%;padding:3px;border:2px solid #eee7ff;background:#fff;overflow:visible}
    .avatar-choice img{width:100%;height:100%;object-fit:cover;border-radius:50%}.avatar-choice.selected{border:4px solid #704be4}
    .avatar-check{position:absolute;right:-2px;bottom:8px;width:27px;height:27px;border-radius:50%;background:#704be4;color:#fff;display:grid;place-items:center;font-weight:900}
    ...
    .avatar-upload small{font-weight:500;color:#8b84a0}.custom-avatar-preview img{width:92px;height:92px;border-radius:50%;object-fit:cover;border:4px solid #704be4}
    ...
    body.theme-pink .version-user-row .user-avatar-button{background:#fff0f5!important;border-color:#ffc9dc!important}
...
function updateStaticAppVersion(){
  const target='PRO 4.9.6 · 29.08.2026';
...
`,
  ...
  topics:()=>collectionView('topics','Duolingo'), themes:()=>themesView(), school:()=>collectionView('school','School'), phrases:()=>collectionView('phrases','Фрази'),
  detail:()=>detailView(),
...
function handleClick(e){if(e.target.closest('.inline-notes'))return;const flash=e.target.closest('#flash');if(flash){flash.classList.toggle('flipped');return}const b=e.target.closest('button');const card=e.target.closest('.word[data-word-id]');if(state.bulkMove&&card){const id=card.dataset.wordId;state.selectedWordIds=state.selectedWordIds.includes(id)?state.selectedWordIds.filter(x=>x!==id):[...state.selectedWordIds,id];render();return}if(!b){if(card){const id=card.dataset.wordId;state.expandedWordId=state.expandedWordId===id?null:id;render()}return}const a=b.dataset.act;
  ...
  else if(a==='close-profile'){document.getElementById('profileModal')?.remove()}
  else if(a==='select-avatar'){localStorage.setItem(avatarStorageKey(),b.dataset.avatar);document.getElementById('profileModal')?.remove();refreshAvatarButton();openProfileModal()}
...
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=4.9.6',{updateViaCache:'none'}).catch(console.error))}
