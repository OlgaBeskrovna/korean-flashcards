...
if(loginVersion)loginVersion.textContent='PRO 5.0.8 · 29.08.2026';
...
async function restoreCloudTheme(){
  try{
    const {data,error}=await supabaseClient.rpc('ensure_my_profile_v5');
    if(error)return;
    const row=Array.isArray(data)?data[0]:data;
    if(row?.theme==='pink'||row?.theme==='purple'){
      localStorage.setItem(APP_THEME_KEY,row.theme);
      applyLoginTheme();
    }
  }catch(error){console.warn('Could not restore cloud theme',error)}
}
